...
editor.defineMode("vi-visual-line", [
  ...
  ["Down", "vi_vline_down"],
  ["Up", "vi_vline_up"],
  ["Left", "vi_vis_line_start"],
  ["Right", "vi_vis_line_end"],
  ["g g", "vi_vis_doc_start"],
  ["G", "vi_vis_doc_end"],
...
function showViCommandError(message: string): void {
  ...
  // Fallback for terminals/builds where the popup API fails.
  if (!ok) {
    editor.setStatus(`E: ${message}`);
  }
}
...
editor.on("prompt_confirmed", async (args) => {
  ...
  if (result.error) {
    showViCommandError(result.error);
...
editor.debug("fresh_pretty_vim vi mode loaded");
...
// Some Vi mode transitions only call setEditorMode internally and do not move the
// cursor. A light render-start fallback keeps the mode chip responsive without
// spamming status-bar writes; values are cached above.
...
editor.debug("fresh_pretty_vim statusline loaded");
